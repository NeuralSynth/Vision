...
function DitheredWaves({
  ...
  const [targetMousePos, setTargetMousePos] = useState<{ x: number; y: number }>({ x: 0, y: 0 });

  useFrame(() => {
    if (enableMouseInteraction) {
      const lerpFactor = 0.15;
      const newX = mousePos.x + (targetMousePos.x - mousePos.x) * lerpFactor;
      const newY = mousePos.y + (targetMousePos.y - mousePos.y) * lerpFactor;
